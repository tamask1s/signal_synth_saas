...
int main() {
    ...
    write_file(verifier_root + "/synsigra-0.13.0-py3-none-any.whl", "wheel");
    ...
    const syn_sig_ra::RouteResponse wheel =
        syn_sig_ra::route_request(
            "GET",
            "/syn_sig_ra/v1/downloads/verifier/synsigra-0.13.0-py3-none-any.whl",
            "/syn_sig_ra",
            "Bearer route-test-key",
            &store,
            download_pack_root
    ...
    require(
        wheel.status == 200 &&
            ...
            wheel.content_disposition.find(
                "synsigra-0.13.0-py3-none-any.whl") != std::string::npos,
    ...
    std::remove((verifier_root + "/synsigra-0.13.0-py3-none-any.whl").c_str());
