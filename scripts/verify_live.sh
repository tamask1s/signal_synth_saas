...
printf '%s' "$verifier" | python3 -c \
  'import json,sys; x=json.load(sys.stdin); assert x["schema_version"]==2 and x["version"]=="0.17.0" and x["generator_included"] is False; assert x["core_git_commit"]=="d4f3f75cb46aeb5bcf7fe9ed700e7d109d00416f"; assert x["challenge_contract"]=="synsigra_challenge_package_v3" and x["scoring_manifest_contract"]=="synsigra_scoring_manifest_v3" and x["verification_protocol_contract"]=="synsigra_verification_protocol_v3"; assert x["submission_contract"]=="synsigra_submission_v1" and x["submission_formats_contract"]=="synsigra_submission_formats_v2"; assert x["measurement_values_contract"]=="synsigra_measurement_values_v2" and x["measurement_truth_contract"]=="synsigra_measurement_truth_v2" and x["measurement_scoring_contract"]=="synsigra_measurement_score_v2" and x["local_verification_contract"]=="synsigra_local_verification_v3"; assert x["verify_example"]=="synsigra-verify challenge submission verification-results --force"'
printf '%s' "$verifier"
printf '\n'
...
case "$openapi" in *"synsigra_core_integration_v8"*"synsigra_challenge_package_v3"*"EvidenceProfile:"*"ChallengeMetadata:"*) ;; *) exit 1 ;; esac
...
printf '%s' "$packs" | python3 -c \
  'import json,sys; x=json.load(sys.stdin); p=x["packs"]; assert len(p)==int(sys.argv[1]); assert {"r_peak_rr_simple_stress_v1","r_peak_rr_snr_ladder_v1","hrv_simple_cases_v1"}<=set(v["pack_id"] for v in p); assert all(v["catalog_version"]=="3.6" and v["catalog_source_sha256"]=="sha256:21f3baee51b6e386962b54a9f30f4223b555f03e055e35cb3259c9c6f7cb9136" and v["integration_contract"]=="synsigra_core_integration_v8" and v["generator_compatibility"]["challenge_package_contract"]=="synsigra_challenge_package_v3" and v["generator_compatibility"]["scoring_manifest_contract"]=="synsigra_scoring_manifest_v3" and v["generator_compatibility"]["submission_contract"]=="synsigra_submission_v1" and v["generator_compatibility"]["verification_protocol_contract"]=="synsigra_verification_protocol_v3" and (not v["evidence_profiles"]["available"] or [q["profile_id"] for q in v["evidence_profiles"]["profiles"]]==["level_1","level_2","level_3"]) for v in p)' \
  "$expected_pack_count"
jobs=$(curl -fsS -H "Authorization: Bearer $key" \
  "$base/v1/jobs?limit=100&offset=0")
usable_current_job=$(printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; print(next((j["job_id"] for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="d4f3f75cb46aeb5bcf7fe9ed700e7d109d00416f" and j.get("package_id") and j.get("artifact_status")!="expired"), ""))')
...
artifact_pair=$(printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; j=next((j for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="d4f3f75cb46aeb5bcf7fe9ed700e7d109d00416f" and j.get("package_id") and j.get("artifact_status")!="expired"), {}); print(j.get("job_id",""), j.get("package_id",""))')
set -- $artifact_pair
artifact_job=${1:-}
artifact_package=${2:-}
