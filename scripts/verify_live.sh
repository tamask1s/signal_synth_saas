...
artifact_pair=$(printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; j=next((j for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="9ea4ff5d018d105966959c43c392316b2353e94d" and j.get("package_id") and j.get("artifact_status")!="expired"), {}); print(j.get("job_id",""), j.get("package_id",""))')
set -- $artifact_pair
artifact_job=${1:-}
artifact_package=${2:-}
