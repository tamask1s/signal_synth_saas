...
printf '%s' "$verifier" | python3 -c \
  'import json,sys; x=json.load(sys.stdin); assert x["schema_version"]==2 and x["version"]=="0.12.0" and x["generator_included"] is False; assert x["core_git_commit"]=="fed2f39355b40627edfbf83d36498f95cc097325"; assert x["challenge_contract"]=="synsigra_challenge_package_v3" and x["scoring_manifest_contract"]=="synsigra_scoring_manifest_v3"; assert x["submission_contract"]=="synsigra_submission_v1" and x["submission_formats_contract"]=="synsigra_submission_formats_v2"; assert x["measurement_values_contract"]=="synsigra_measurement_values_v2" and x["measurement_truth_contract"]=="synsigra_measurement_truth_v2" and x["measurement_scoring_contract"]=="synsigra_measurement_score_v2" and x["local_verification_contract"]=="synsigra_local_verification_v3"; assert x["verify_example"]=="synsigra-verify challenge submission verification-results --force"'
printf '%s' "$verifier"
printf '\n'
...
printf '%s' "$packs" | python3 -c \
  'import json,sys; x=json.load(sys.stdin); p=x["packs"]; assert len(p)==18; assert all(v["catalog_version"]=="3.0" and v["catalog_source_sha256"]=="sha256:491598ee6f3296af37c9e4943178d7288da0adbafb75af87625f53e7ce3c8612" and v["integration_contract"]=="synsigra_core_integration_v7" and v["generator_compatibility"]["challenge_package_contract"]=="synsigra_challenge_package_v3" and v["generator_compatibility"]["scoring_manifest_contract"]=="synsigra_scoring_manifest_v3" and v["generator_compatibility"]["submission_contract"]=="synsigra_submission_v1" and v["generator_compatibility"]["verification_protocol_contract"]=="synsigra_verification_protocol_v2" for v in p)'
jobs=$(curl -fsS -H "Authorization: Bearer $key" \
  "$base/v1/jobs?limit=100&offset=0")
usable_current_job=$(printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; print(next((j["job_id"] for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="fed2f39355b40627edfbf83d36498f95cc097325" and j.get("package_id") and j.get("artifact_status")!="expired"), ""))')
...
artifact_pair=$(printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; j=next((j for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="fed2f39355b40627edfbf83d36498f95cc097325" and j.get("package_id") and j.get("artifact_status")!="expired"), {}); print(j.get("job_id",""), j.get("package_id",""))')
set -- $artifact_pair
artifact_job=${1:-}
artifact_package=${2:-}
