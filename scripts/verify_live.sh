...
printf '%s' "$mcp_recommend" | python3 -c \
  'import json,sys; r=json.load(sys.stdin)["result"]["structuredContent"]; assert set(r["interpreted_targets"])=={"r_peak","rr_interval","hrv"}; assert r["candidates"] and r["recommended_workflow"] in {"inspect_top_curated_pack_then_create_job","use_custom_authoring_for_unmet_requirements"}'
verifier=$(curl -fsS -H "Authorization: Bearer $key" "$base/v1/downloads/verifier")
printf '%s' "$verifier" | python3 -c \
  'import json,sys; x=json.load(sys.stdin); assert x["schema_version"]==2 and x["version"]=="0.18.0" and x["generator_included"] is False; assert x["core_git_commit"]=="e089196a65dba7567b5f7db2ae020e0b4327f40f"; assert x["challenge_contract"]=="synsigra_challenge_package_v3" and x["scoring_manifest_contract"]=="synsigra_scoring_manifest_v3" and x["verification_protocol_contract"]=="synsigra_verification_protocol_v4"; assert x["submission_contract"]=="synsigra_submission_v1" and x["submission_formats_contract"]=="synsigra_submission_formats_v2"; assert x["measurement_values_contract"]=="synsigra_measurement_values_v2" and x["measurement_truth_contract"]=="synsigra_measurement_truth_v2" and x["measurement_scoring_contract"]=="synsigra_measurement_score_v2" and x["local_verification_contract"]=="synsigra_local_verification_v4"; assert x["verify_example"]=="synsigra-verify challenge submission verification-results --force"'
printf '%s' "$verifier"
printf '\n'
...
expected_pack_count=$(python3 -c \
  'import json,sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["pack_count"])' \
  "$repo_dir/packs/curated_pack_metadata_v1.catalog")
printf '%s' "$packs" | python3 -c \
  'import json,sys; x=json.load(sys.stdin); p=x["packs"]; assert len(p)==int(sys.argv[1]); assert {"r_peak_rr_simple_stress_v1","r_peak_rr_snr_ladder_v1","hrv_simple_cases_v1"}<=set(v["pack_id"] for v in p); assert all(v["catalog_version"]=="3.8" and v["catalog_source_sha256"]=="sha256:1d1bc6c4c8a25d942bc4c6ef786efc1d9d7b67010a769a925be89648b1496ec6" and v["integration_contract"]=="synsigra_core_integration_v9" and v["generator_compatibility"]["challenge_package_contract"]=="synsigra_challenge_package_v3" and v["generator_compatibility"]["scoring_manifest_contract"]=="synsigra_scoring_manifest_v3" and v["generator_compatibility"]["submission_contract"]=="synsigra_submission_v1" and v["generator_compatibility"]["verification_protocol_contract"]=="synsigra_verification_protocol_v4" and (not v["evidence_profiles"]["available"] or ([q["profile_id"] for q in v["evidence_profiles"]["profiles"]]==["level_1","level_2","level_3"] and all(q["document"]["evidence_basis"]["classification"]=="synsigra_pre_specified_engineering_gates" and q["document"]["evidence_basis"]["direct_standard_thresholds"] is False and q["document"]["evidence_basis"]["sources"] for q in v["evidence_profiles"]["profiles"]))) for v in p)' \
  "$expected_pack_count"
jobs=$(curl -fsS -H "Authorization: Bearer $key" \
  "$base/v1/jobs?limit=100&offset=0")
usable_current_job=$(printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; print(next((j["job_id"] for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="e089196a65dba7567b5f7db2ae020e0b4327f40f" and j.get("package_id") and j.get("artifact_status")!="expired"), ""))')
if [ -z "$usable_current_job" ]; then
  printf 'check=create-current-core-smoke-job\n'
  python3 "$repo_dir/scripts/stress_live_packs.py" \
    --base-url "$base" \
    --pack-id r_peak_rr_simple_stress_v1 \
    --keep-jobs
  jobs=$(curl -fsS -H "Authorization: Bearer $key" \
    "$base/v1/jobs?limit=100&offset=0")
fi
printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; current=[j for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="e089196a65dba7567b5f7db2ae020e0b4327f40f"]; assert current and any(j.get("package_id") and j.get("artifact_status")!="expired" for j in current); assert all(j.get("integration_contract")=="synsigra_core_integration_v9" and j.get("generator_build_identity")=="signal_synth/"+j["generator_git_commit"] and j.get("generator_binary_sha256","").startswith("sha256:") and len(j["generator_binary_sha256"])==71 and j.get("challenge",{}).get("challenge_contract")=="synsigra_challenge_package_v3" and j.get("challenge",{}).get("scoring_manifest_contract")=="synsigra_scoring_manifest_v3" and j.get("challenge",{}).get("submission_contract")=="synsigra_submission_v1" and j.get("challenge",{}).get("submission_formats_contract")=="synsigra_submission_formats_v2" and j.get("challenge",{}).get("measurement_values_contract")=="synsigra_measurement_values_v2" and j.get("challenge",{}).get("measurement_truth_contract")=="synsigra_measurement_truth_v2" and j.get("challenge",{}).get("measurement_scoring_contract")=="synsigra_measurement_score_v2" and j.get("challenge",{}).get("local_verification_contract")=="synsigra_local_verification_v4" and j.get("challenge",{}).get("verification",{}).get("mode") in ("evidence","diagnostic") and (j.get("challenge",{}).get("verification",{}).get("mode")!="evidence" or (j["challenge"]["verification"]["protocol"]["evidence_basis"]["classification"]=="synsigra_pre_specified_engineering_gates" and j["challenge"]["verification"]["protocol"]["evidence_basis"]["direct_standard_thresholds"] is False and j["challenge"]["verification"]["protocol"]["evidence_basis"]["sources"])) and j.get("challenge",{}).get("integrity",{}).get("ok") is True for j in current)'
...
artifact_pair=$(printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; j=next((j for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="e089196a65dba7567b5f7db2ae020e0b4327f40f" and j.get("package_id") and j.get("artifact_status")!="expired"), {}); print(j.get("job_id",""), j.get("package_id",""))')
set -- $artifact_pair
artifact_job=${1:-}
artifact_package=${2:-}
