...
printf '%s' "$packs" | python3 -c \
  'import json,sys; x=json.load(sys.stdin); p=x["packs"]; assert len(p)==int(sys.argv[1]); assert {"r_peak_rr_simple_stress_v1","r_peak_rr_snr_ladder_v1","hrv_simple_cases_v1"}<=set(v["pack_id"] for v in p); assert all(v["catalog_version"]=="3.7" and v["catalog_source_sha256"]=="sha256:6862313147dcdeab429e1972af62b81e9a7c4025c6707ee752c3126f5f8b556a" and v["integration_contract"]=="synsigra_core_integration_v8" and v["generator_compatibility"]["challenge_package_contract"]=="synsigra_challenge_package_v3" and v["generator_compatibility"]["scoring_manifest_contract"]=="synsigra_scoring_manifest_v3" and v["generator_compatibility"]["submission_contract"]=="synsigra_submission_v1" and v["generator_compatibility"]["verification_protocol_contract"]=="synsigra_verification_protocol_v3" and (not v["evidence_profiles"]["available"] or [q["profile_id"] for q in v["evidence_profiles"]["profiles"]]==["level_1","level_2","level_3"]) for v in p)' \
  "$expected_pack_count"
jobs=$(curl -fsS -H "Authorization: Bearer $key" \
  "$base/v1/jobs?limit=100&offset=0")
usable_current_job=$(printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; print(next((j["job_id"] for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="3454c03cafdad2947446008f9d29ed011102dfe7" and j.get("package_id") and j.get("artifact_status")!="expired"), ""))')
...
artifact_pair=$(printf '%s' "$jobs" | python3 -c \
  'import json,sys; jobs=json.load(sys.stdin)["jobs"]; j=next((j for j in jobs if j.get("status")=="succeeded" and j.get("generator_git_commit")=="3454c03cafdad2947446008f9d29ed011102dfe7" and j.get("package_id") and j.get("artifact_status")!="expired"), {}); print(j.get("job_id",""), j.get("package_id",""))')
set -- $artifact_pair
artifact_job=${1:-}
artifact_package=${2:-}
