...
printf '%s' "$verifier" | python3 -c \
  'import json,sys; x=json.load(sys.stdin); assert x["schema_version"]==2 and x["version"]=="0.16.0" and x["generator_included"] is False; assert x["core_git_commit"]=="d4f3f75cb46aeb5bcf7fe9ed700e7d109d00416f"; assert x["challenge_contract"]=="synsigra_challenge_package_v3" and x["scoring_manifest_contract"]=="synsigra_scoring_manifest_v3"; assert x["submission_contract"]=="synsigra_submission_v1" and x["submission_formats_contract"]=="synsigra_submission_formats_v2"; assert x["measurement_values_contract"]=="synsigra_measurement_values_v2" and x["measurement_truth_contract"]=="synsigra_measurement_truth_v2" and x["measurement_scoring_contract"]=="synsigra_measurement_score_v2" and x["local_verification_contract"]=="synsigra_local_verification_v3"; assert x["verify_example"]=="synsigra-verify challenge submission verification-results --force"'
printf '%s' "$verifier"
printf '\n'
